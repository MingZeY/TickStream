/*
TickStream
规则：bucket总量10000CPU，每tick最多可用500CPU
Game.cpu.getUsed()返回当前tick此时使用的cpu
...
module.exports = {
    ...
    deploy: function (isCompulsive) {
        ...
        Memory.TickStream = {};//根
        Memory.TickStream.Version = "1.4";//TickStream版本号
    ...
    WeightSystem: {
        ...
        returnNextTickResult: function () {
            ...
            */
            ...
            let runTimeLimit = 20;
            try {
                runTimeLimit = require('tickStreamAlgorithm')(FunList, NextQueue, Algorithm);
            } catch (e) {
                let NextQueue = new global.tickStream.LinkedQueue.creat();
                console.log(e.stack + '\nTickStream温馨提示：您的算法出现语法错误');
                let view = new RoomVisual();
