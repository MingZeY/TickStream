/*
文件名：tickStreamMonutExtension.js
描述：tickStream框架逻辑文件
...
module.exports = function (funList) {
    ...
    if (typeof (funList) == 'undefined' || funList.length == 0) {
        let view = new RoomVisual();
        view.text("TickStream已部署", 25, 25, { font: 2, color: "#4CAF50", stroke: "#fff", strokeWidth: 0.5 });
        view.text("请按要求注册您的方法到TickStream", 25, 27, { stroke: "#000" });
        return 0;
    }
    ...
    if (!global.hasTickStream) {
        ...
        //挂载一个判断是否有重新挂载刷新的参数用于提示Algorithm，只会持续1tick
        global.tickStream.isRefresh = true;
    } else {
        global.tickStream.isRefresh = false;
    }
    ...
    */
    ...
    let Register = TickStream.Register;//注册表对象
     ...
     */
    //定时清理注册表清除多余项
    if (Game.time % 20) Register.refresh();
